...
{
	m_szClass = L"CXUI_Button";
	m_szName = L"";
	m_bCreate = false;
}

CXUI_Button::CXUI_Button(std::wstring& szName)
{
	m_szClass = L"CXUI_Button";
	m_szName = szName;
	m_bCreate = false;
...
CXUI_Button::CXUI_Button(const wchar_t* szName)
{
	m_szClass = L"CXUI_Button";
	m_szName = szName;
	m_bCreate = false;
...
bool CXUI_Button::CreateCtrl(void)
{
	if(m_bCreate)
		return m_bCreate;
	...
	if(m_nCtrlID != 0)
	{
		...
		if(m_hParent != NULL) 
		{
			...
			HWND hWnd = m_Button.Create(m_hParent, ATL::_U_RECT(&rect), CW2CT(m_szText.c_str()),
						m_nStyle, m_nExtStyle, ATL::_U_MENUorID(m_nCtrlID), NULL);

			if(hWnd != NULL)
			{
				m_Button.SetDlgCtrlID(m_nCtrlID);
				m_bCreate = true;
			}
		}
#endif
...
void CXUI_Button::SetText(std::wstring& szText)
{
	...
	{
#ifdef __USE_WTL__
		m_Button.SetWindowText(CW2CT(m_szText.c_str()));
#endif
	}
}

void CXUI_Button::SetText(const wchar_t* szText)
{
	CXUI_CtrlBase::SetText(szText);
...
std::wstring CXUI_Button::GetText(void)
{
#ifdef __USE_WTL__
	CComBSTR	bstr = NULL;
	if(m_Button.GetWindowText(&bstr))
	{
		USES_CONVERSION;
...
CXUI_Button& CXUI_Button::operator = (const CXUI_FormTrait& ctrlTrait)
{
...
void CXUI_Button::Assign(CXOS_ClassObject* pObj)
{
	wchar_t sTemp[1024];
