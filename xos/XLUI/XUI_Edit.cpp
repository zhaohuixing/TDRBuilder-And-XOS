...
{
	m_szClass = L"CXUI_Edit";
	m_szName = L"";
	m_bCreate = false;
}

CXUI_Edit::CXUI_Edit(std::wstring& szName)
{
	m_szClass = L"CXUI_Edit";
	m_szName = szName;
	m_bCreate = false;
...
CXUI_Edit::CXUI_Edit(const wchar_t* szName)
{
	m_szClass = L"CXUI_Edit";
	m_szName = szName;
	m_bCreate = false;
...
bool CXUI_Edit::CreateCtrl(void)
{
	if(m_bCreate)
		return m_bCreate;
	...
	if(m_nCtrlID != 0)
	{
		...
		if(m_hParent != NULL) 
		{
			...
			HWND hWnd = m_Edit.Create(m_hParent, ATL::_U_RECT(&rect), CW2CT(m_szText.c_str()),
						m_nStyle, m_nExtStyle, ATL::_U_MENUorID(m_nCtrlID), NULL);

			if(hWnd != NULL)
			{
				m_Edit.SetDlgCtrlID(m_nCtrlID);
				m_bCreate = true;
			}
		}
#endif
...
void CXUI_Edit::SetText(std::wstring& szText)
{
	...
	{
#ifdef __USE_WTL__
		m_Edit.SetWindowText(CW2CT(m_szText.c_str()));
#endif
	}
}

void CXUI_Edit::SetText(const wchar_t* szText)
{
	CXUI_CtrlBase::SetText(szText);
...
std::wstring CXUI_Edit::GetText(void)
{
#ifdef __USE_WTL__
	CComBSTR	bstr = NULL;
	if(m_Edit.GetWindowText(&bstr))
	{
		USES_CONVERSION;
...
CXUI_Edit& CXUI_Edit::operator = (const CXUI_FormTrait& ctrlTrait)
{
...
void CXUI_Edit::Assign(CXOS_ClassObject* pObj)
{
	wchar_t sTemp[1024];
