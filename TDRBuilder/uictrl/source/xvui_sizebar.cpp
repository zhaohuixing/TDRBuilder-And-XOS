...
CSCBArray CVUI_CSizeCtrlBar::m_arrBars; // static member

IMPLEMENT_DYNAMIC(CVUI_CSizeCtrlBar, CControlBar);

CVUI_CSizeCtrlBar::CVUI_CSizeCtrlBar()
{
    m_szMin = CSize(33, 32);
...
BEGIN_MESSAGE_MAP(CVUI_CSizeCtrlBar, CControlBar)
    //{{AFX_MSG_MAP(CVUI_CSizeCtrlBar)
    ON_WM_CREATE()
    ON_WM_PAINT()
    ON_WM_NCPAINT()
    ON_WM_NCCALCSIZE()
...
BOOL CVUI_CSizeCtrlBar::Create(LPCTSTR lpszWindowName, CWnd* pParentWnd,
...
/////////////////////////////////////////////////////////////////////////
// CVUI_CSizeCtrlBar message handlers

int CVUI_CSizeCtrlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
BOOL CVUI_CSizeCtrlBar::DestroyWindow() 
{
...
const BOOL CVUI_CSizeCtrlBar::IsFloating() const
{
    return !IsHorzDocked() && !IsVertDocked();
}

const BOOL CVUI_CSizeCtrlBar::IsHorzDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

const BOOL CVUI_CSizeCtrlBar::IsVertDocked() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_LEFT ||
        m_nDockBarID == AFX_IDW_DOCKBAR_RIGHT);
}

const BOOL CVUI_CSizeCtrlBar::IsSideTracking() const
{
...
CSize CVUI_CSizeCtrlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    ...
    // dirty cast - using CVUI_CBDockBar to access protected CDockBar members
    CVUI_CBDockBar* pDockBar = (CVUI_CBDockBar*) m_pDockBar;
...
CSize CVUI_CSizeCtrlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void CVUI_CSizeCtrlBar::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
...
// Mouse Handling
//
void CVUI_CSizeCtrlBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
    if (m_pDockBar != NULL)
    {
        // start the drag
...
void CVUI_CSizeCtrlBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
    if (m_pDockBar != NULL)
    {
        // toggle docking
...
void CVUI_CSizeCtrlBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
    if (IsFloating())
    {
...
void CVUI_CSizeCtrlBar::OnNcLButtonUp(UINT nHitTest, CPoint point) 
{
    if (nHitTest == HTCLOSE)
...
void CVUI_CSizeCtrlBar::OnLButtonUp(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
        StopTracking();
...
void CVUI_CSizeCtrlBar::OnRButtonDown(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
        StopTracking();
...
void CVUI_CSizeCtrlBar::OnMouseMove(UINT nFlags, CPoint point) 
{
    if (m_bTracking)
...
void CVUI_CSizeCtrlBar::OnCaptureChanged(CWnd *pWnd) 
{
...
void CVUI_CSizeCtrlBar::OnNcCalcSize(BOOL bCalcValidRects,
...
void CVUI_CSizeCtrlBar::OnNcPaint() 
{
...
void CVUI_CSizeCtrlBar::NcPaintGripper(CDC* pDC, CRect rcClient)
{
...
void CVUI_CSizeCtrlBar::OnPaint()
{
    // overridden to skip border painting based on clientrect
    CPaintDC dc(this);
}

UINT CVUI_CSizeCtrlBar::OnNcHitTest(CPoint point)
{
    if (IsFloating())
...
void CVUI_CSizeCtrlBar::OnSettingChange(UINT uFlags, LPCTSTR lpszSection) 
{
...
/////////////////////////////////////////////////////////////////////////
// CVUI_CSizeCtrlBar implementation helpers

void CVUI_CSizeCtrlBar::StartTracking(UINT nHitTest)
{
    SetCapture();
    ...
    else
    {
        ...
        CVUI_CSizeCtrlBar* pBar = arrSCBars[i +
...
void CVUI_CSizeCtrlBar::StopTracking()
{
...
void CVUI_CSizeCtrlBar::OnTrackUpdateSize(CPoint& point)
{
    ASSERT(!IsFloating());
...
void CVUI_CSizeCtrlBar::OnTrackInvertTracker()
{
    ASSERT(m_bTracking);
...
BOOL CVUI_CSizeCtrlBar::GetEdgeRect(CRect rcWnd, UINT nHitTest,
                                    CRect& rcEdge)
{
    rcEdge = rcWnd;
    if (m_dwSCBStyle & SCBS_SHOWEDGES)
...
UINT CVUI_CSizeCtrlBar::GetEdgeHTCode(int nEdge)
{
...
void CVUI_CSizeCtrlBar::GetRowInfo(int& nFirst, int& nLast, int& nThis)
{
...
void CVUI_CSizeCtrlBar::GetRowSizingBars(CSCBArray& arrSCBars)
{
    arrSCBars.RemoveAll();
    ...
    for (int i = nFirst; i <= nLast; i++)
    {
        ...
        if (FindSizingBar(pBar) >= 0)
            arrSCBars.Add((CVUI_CSizeCtrlBar*)pBar);
    }
}

const int CVUI_CSizeCtrlBar::FindSizingBar(CControlBar* pBar) const
{
...
BOOL CVUI_CSizeCtrlBar::NegociateSpace(int nLengthAvail, BOOL bHorz)
{
...
void CVUI_CSizeCtrlBar::AlignControlBars()
{
    int nFirst, nLast, nThis;
    GetRowInfo(nFirst, nLast, nThis);
    ...
    {
        m_pDockSite->DelayRecalcLayout();
        TRACE("CVUI_CSizeCtrlBar DelayRecalcLayout\n");
    }
}

void CVUI_CSizeCtrlBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
    ...
    BOOL bWasPushed = m_biHide.m_bPushed;
    m_biHide.m_bPushed = bHit && bLButtonDown;

    BOOL bWasRaised = m_biHide.m_bRaised;
    m_biHide.m_bRaised = bHit && !bLButtonDown;

    bNeedPaint |= (m_biHide.m_bPushed ^ bWasPushed) || 
                  (m_biHide.m_bRaised ^ bWasRaised);

    if (bNeedPaint)
        SendMessage(WM_NCPAINT);
}

void CVUI_CSizeCtrlBar::LoadState(LPCTSTR lpszProfileName)
{
    ASSERT_VALID(this);
...
void CVUI_CSizeCtrlBar::SaveState(LPCTSTR lpszProfileName)
{
...
void CVUI_CSizeCtrlBar::GlobalLoadState(LPCTSTR lpszProfileName)
{
    for (int i = 0; i < m_arrBars.GetSize(); i++)
        ((CVUI_CSizeCtrlBar*) m_arrBars[i])->LoadState(lpszProfileName);
}

void CVUI_CSizeCtrlBar::GlobalSaveState(LPCTSTR lpszProfileName)
{
    for (int i = 0; i < m_arrBars.GetSize(); i++)
        ((CVUI_CSizeCtrlBar*) m_arrBars[i])->SaveState(lpszProfileName);
}

/////////////////////////////////////////////////////////////////////////
// CVUI_CBButton

CVUI_CBButton::CVUI_CBButton()
{
    m_bRaised = FALSE;
    m_bPushed = FALSE;
}

void CVUI_CBButton::Paint(CDC* pDC)
{
    CRect rc = GetRect();
    ...
    pDC->TextOut(m_ptOrg.x + 2, m_ptOrg.y + 2, CString(_T("r"))); // x-like
