...
BOOL CVUI_VarEditDlg::AddNewLocalVar(void)
{
	BOOL bRet = FALSE;
	if(m_State != enVAR_LOCALNEW && m_State != enVAR_LOCALEDIT)
		return bRet;
	...
	{
		bRet = AddNewLocalVarNoArray(nType);
	}
	else if(nType == XVSM_DT_ARRAY)
	{
		bRet = AddNewLocalVarArray();
	}

	return bRet;
}

BOOL CVUI_VarEditDlg::UpdateExistLocalVar(void)
{
	BOOL bRet = FALSE;
	if(m_State != enVAR_LOCALNEW && m_State != enVAR_LOCALEDIT)
		return bRet;
	...
	{
		bRet = UpdateExistLocalVarNoArray(nType);
	}
	else
	{
		bRet = UpdateExistLocalVarArray();
	}
	return bRet;
}

BOOL CVUI_VarEditDlg::AddNewLocalVarNoArray(int nType)
{
	BOOL bRet = FALSE;
	...
	if(nType == XVSM_DT_FLOAT)
	{
		...
		if(m_pMMan->m_Module.AddLocalVar(pDbl, wszVar))
			bRet = TRUE;
	}
	else if(nType == XVSM_DT_INTEGER)
	{
		...
		if(m_pMMan->m_Module.AddLocalVar(pInt, wszVar))
			bRet = TRUE;
	}
	else if(nType == XVSM_DT_BYTE)
	{
		...
		if(m_pMMan->m_Module.AddLocalVar(pByte, wszVar))
			bRet = TRUE;
	}
	else if(nType == XVSM_DT_BOOL)
	{
		...
		if(m_pMMan->m_Module.AddLocalVar(pBool, wszVar))
			bRet = TRUE;
	}
...
BOOL CVUI_VarEditDlg::UpdateExistLocalVarNoArray(int nType)
{
	BOOL bRet = FALSE;
	if((m_State != enVAR_LOCALNEW && m_State != enVAR_LOCALEDIT) || nType == XVSM_DT_ARRAY)
		return bRet;
	...
	if(nType == XVSM_DT_FLOAT)
	{
		...
		m_pMMan->m_Module.RemoveLocalVar(m_OldVarName);
		CVSB_Float* pDbl = new CVSB_Float(dVal, wszVar);
		if(m_pMMan->m_Module.AddLocalVar(pDbl, wszVar))
			bRet = TRUE;
	}
...
BOOL CVUI_VarEditDlg::AddNewLocalVarArray(void)
{
	BOOL bRet = FALSE;
	if(m_State != enVAR_LOCALNEW && m_State != enVAR_LOCALEDIT)
		return bRet;
	...
	if(m_pMMan->m_Module.AddLocalVar(pArray, wszVar))
	{
		bRet = TRUE;
...
BOOL CVUI_VarEditDlg::UpdateExistLocalVarArray(void)
{
	BOOL bRet = FALSE;
	if(m_State != enVAR_LOCALNEW && m_State != enVAR_LOCALEDIT)
		return bRet;
...
BOOL CVUI_VarEditDlg::LoadExistLocalVar(void)
{
	BOOL bRet = FALSE;
	if(m_pMMan)
	{
		...
		iter = m_pMMan->m_Module.m_LocalVars.m_ObjTbl.find(m_OldVarName);
		if(iter != m_pMMan->m_Module.m_LocalVars.m_ObjTbl.end())
		{
			pData = iter->second;
			...
			if(pData)
			{
				...
				else
				{
					...
					nAEType =((CVSB_Array*)pData)->At(0)->GetDataTypeIndex();
