...
bool CVSB_ExpFactory::_down2op(enXVSB_OPERATION ot)
{
	bool bRet = false;
	...
	if(mLvl == enXVSB_LEVEL2)  // Prev OP: None; New OP: *,/      
	{
		...
		//Add the variable to the cursor node
		if(pTemp)
		{
	...
	else if(mLvl == enXVSB_LEVEL3)// Prev OP: +, -; New OP: level 3 operations      
	{
		if(ot == enXVSB_FACTORIAL) // Replace the current data on the top of filter
		{
			...
			{
				// Create the level 1 temperary node
				pMTemp1 = mBuilder((CVSB_hENode)m_hCusor->m_pParent, enXVSB_LEVEL1, enXVSB_NONE);
		...
		else
		{
			...
			if(pTemp && (ot == enXVSB_LSHIFT || ot == enXVSB_RSHIFT || ot == enXVSB_AND || ot == enXVSB_EXOR || ot == enXVSB_INOR))
			{
				...
				pMTemp1 = mBuilder((CVSB_hENode)m_hCusor->m_pParent, enXVSB_LEVEL1, prevOP);
				m_hCusor->m_pParent->AddSubOp(pMTemp1);
...
bool CVSB_ExpFactory::_up2op(enXVSB_OPERATION ot)
{
	bool bRet = false;
	bool bOK = 	(m_hExpress && m_hCusor && m_opFilter.IsValid() && m_bInProcess);
	...
	else if(mLvl == enXVSB_LEVEL1) // Prev OP: level 3 OPs; New OP: +,-      
	{
		prevOP = m_opFilter.PopType();
		pTemp = (CVSB_hENode)m_opFilter.PopVar();
