...
CVSB_Equation::CVSB_Equation(CVSB_OwnerBase* hOwner)
{
	m_hOwner = hOwner;
	m_LVarStr.clear();
	m_RExpStr.clear();
...
CVSB_Equation::CVSB_Equation(CVSB_OwnerBase* hOwner, const std::wstring& szLStr, const std::wstring& szRExp)
	: m_LVarStr(szLStr), m_RExpStr(szRExp)
{
	m_hOwner = hOwner;
	m_RFormula = NULL;
	m_errCode = enXVSB_ERRNONE;
...
CVSB_Equation::CVSB_Equation(CVSB_OwnerBase* hOwner, const wchar_t* szLStr, const wchar_t* szRExp)
	: m_LVarStr(szLStr), m_RExpStr(szRExp)
{
	m_hOwner = hOwner;
	m_RFormula = NULL;
...
CVSB_Equation::CVSB_Equation(const CVSB_Equation& hEqua)
{
	m_hOwner = hEqua.m_hOwner;
...
CVSB_Equation::~CVSB_Equation()
{
	CleanAll();
	m_hOwner = NULL;
}

void CVSB_Equation::Reset(void)
{
	CXOS_EUnitRelease efree;
	efree(m_RFormula);
	m_RFormula = NULL;
	m_errCode = enXVSB_ERRNONE;
	m_LVar.Reset();
...
void CVSB_Equation::CleanAll(void)
{
	m_LVarStr.clear();
	m_RExpStr.clear();
	CXOS_EUnitRelease efree;
...
bool CVSB_Equation::Startup(void)
{
	bool bRet = false;
...
bool CVSB_Equation::Shutdown(void)
{
	bool bRet = true;
	Reset();
	return bRet;
...
void CVSB_Equation::SetLVarStr(const std::wstring& szLStr)
{
	m_LVarStr = szLStr;
}

void CVSB_Equation::SetLVarStr(const wchar_t* szLStr)
{
	m_LVarStr = szLStr;
}

std::wstring CVSB_Equation::GetLVarStr(void)
{
	return m_LVarStr;
}

const wchar_t* CVSB_Equation::GetLVarStr(void) const
{
	return m_LVarStr.c_str();
}

void CVSB_Equation::SetRExpStr(const std::wstring& szRExp)
{
	m_RExpStr = szRExp;
}

void CVSB_Equation::SetRExpStr(const wchar_t* szRExp)
{
	m_RExpStr = szRExp;
}

std::wstring CVSB_Equation::GetRExpStr(void)
{
	return m_RExpStr;
}

const wchar_t* CVSB_Equation::GetRExpStr(void) const
{
	return m_RExpStr.c_str();
}

bool CVSB_Equation::SetOwner(CVSB_OwnerBase* hOwner)
{
	bool bRet = false;
	if(hOwner == NULL)
	{
...
IVSB_DataBase* CVSB_Equation::GetLVarData(void) const
{
	return m_LVar.GetParamData();
}

CVSB_hENode CVSB_Equation::GetRFormula(void) const
{
	return m_RFormula;
}

enXVSB_ERRORCODE CVSB_Equation::GetError(void) const
{
	return m_errCode;
}

std::wstring CVSB_Equation::GetErrorInfo(void)
{
	return m_errString;
}
	
const wchar_t* CVSB_Equation::GetErrorInfo(void) const
{
	return m_errString.c_str();
}

bool CVSB_Equation::_processleftvar(void)
{
	bool bRet = false;
	bool bArray = false;
...
bool CVSB_Equation::_createformula(void)
{
	bool bRet = false;
	wchar_t  szText[2048];
	memset(szText, 0, 2048*sizeof(wchar_t));
...
bool CVSB_Equation::_attachlvar(void)
{
	bool bRet = false;
...
bool CVSB_Equation::ReSetup(void)
{
	bool bRet = true;
...
std::wstring CVSB_Equation::GetCPPFmtRExpStr(void)
{
...
std::wstring CVSB_Equation::GetEquation(void)
{
	std::wstring szDes;
...
std::wstring CVSB_Equation::GetCPPFmtEquation(void)
{
	std::wstring szDes;
