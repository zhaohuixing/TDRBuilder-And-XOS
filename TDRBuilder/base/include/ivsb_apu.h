...
//
// File Name: ivsb_apu.h
//
// Description: The definition of the abstract interface of processor unit
//
// Author: Zhaohui Xing
//
...
// 1.    01/20/2005: Create	
//
#ifndef __IVSB_APU_H__
#define __IVSB_APU_H__
...
class IVSB_ProcessorBase  // : public IVSB_Base
{
public:
	IVSB_ProcessorBase(CVSB_OwnerBase* pOwner = NULL, int nID = 0)
	{
		m_pOwner = pOwner; 
		m_puID = nID;
		m_PrevPUlist.clear();
...
public:
	virtual void Attach(CVSB_OwnerBase*	pOwner){m_pOwner = pOwner;}
	void Detach(void){m_pOwner = NULL;}
	CVSB_OwnerBase* GetOwner(void) const {return m_pOwner;}
	...
	virtual bool Clone(IVSB_ProcessorBase** ppPU) = 0;
...
typedef std::map<int, IVSB_ProcessorBase*>				CVSB_PUMap;
typedef TVSB_PairDeletor<CVSB_PUMap::value_type>		CVSB_PUMapDelete;
typedef TVSB_Dealloc<CVSB_PUMap, CVSB_PUMapDelete>		CVSB_PUMapRelease;
